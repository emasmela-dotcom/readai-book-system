...
export default async function GenrePage({
    ...
    return (
      <main className="min-h-screen bg-[#0e0c0a] px-5 py-10 text-[#e8e4df] md:px-8 md:py-14">
        <div className="mx-auto max-w-6xl">
          <Link href="/es/genres" className="text-xs uppercase tracking-[0.2em] text-[#c9a96e] hover:underline">
  ...
  const prevPageHref =
    page <= 1 ? null : page - 1 === 1 ? `/es/genres/${aisle.id}` : `/es/genres/${aisle.id}?page=${page - 1}`
  const nextPageHref =
    page >= totalPages || sourceShelf.total === 0
      ? null
      : `/es/genres/${aisle.id}?page=${page + 1}`
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-8 border-b border-white/10 pb-6">
          ...
          <p className="mt-2 text-xs text-[#eadfce]/80">
            Cada título aquí se abre en ReadAI. La búsqueda en inicio también encuentra libros nuevos.
          </p>
        ...
        {sourceShelf.books.length === 0 ? (
          <p className="text-sm text-[#e8e4df]/80">
            ...
            or another{' '}
            <Link href="/es/genres" className="text-[#c9a96e] hover:underline">
              salas de lectura
            </Link>
            .
          </p>
