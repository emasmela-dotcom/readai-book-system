...
export const metadata: Metadata = {
  title: getDictionary('es').auth.signUpTitle,
  description: getDictionary('es').auth.signUpBody,
  alternates: {
    canonical: '/es/sign-up',
    ...
    <main className="min-h-screen bg-[#0e0c0a] px-5 py-10 text-[#f5f2ed] md:px-8 md:py-14">
      <div className="mx-auto max-w-md">
        ...
        <header className="border-b border-white/10 pb-6">
          <p className="text-[11px] uppercase tracking-[0.3em] text-[#c9a96e]">{t.auth.signUpEyebrow}</p>
          <h1 className="mt-2 font-serif text-3xl text-[#f5f2ed]">{t.auth.signUpTitle}</h1>
          <p className="mt-3 text-sm leading-relaxed text-[#eadfce]">{t.auth.signUpBody}</p>
        </header>
